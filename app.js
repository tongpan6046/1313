...
// **重設遊戲，應該清除玩家資料**
function resetGame() {
    let transactionScores = db.transaction(["scores"], "readwrite");
    let storeScores = transactionScores.objectStore("scores");
    storeScores.clear();

    let transactionPlayers = db.transaction(["players"], "readwrite");
    let storePlayers = transactionPlayers.objectStore("players");
    storePlayers.clear(); // 清除玩家資料

    lastAction = "resetGame"; // 記錄動作
    loadPlayers(); // 重新載入玩家清單
    loadHistory();
}
